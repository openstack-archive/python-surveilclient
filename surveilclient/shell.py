...
class SurveilShell(object):
    ...
    def main(self, argv):
        ...
        parser = self.get_subcommand_parser(options.surveil_api_version)
        if not argv or (options.help and not args):
            self.do_help()
            return 0

        options = parser.parse_args(argv)
        if options.help or options.func == self.do_help:
            self.do_help(options)
            return 0

        if options.func == self.do_bash_completion:
            self.do_bash_completion(options)
            return 0

        endpoint = options.surveil_api_url
        if not endpoint:
            raise exc.CommandError("you must specify a Surveil API URL"
                                   " via either --surveil-api-url or"
                                   " env[SURVEIL_API_URL]")
        client = surveil_client.Client(endpoint, version=options.surveil_api_version)
        return options.func(client, options)
